...
# ########################################################################### #
app = nap.core.Plugin(description="NAGIOS XRootD probe",
...
def metricLsDir(args, io):
    ...
    try:
        ...
        io.summary = 'Storage Path[%s] Directory successfully listed' % str(args.endpoint)
        io.status = nap.OK
    ...
    for endpt in _fileDictionary.keys():
        ...
        try:
            ...
            if filecmp.cmp(_fileTest, _fileTestIn):
                ...
                io.status = nap.OK
                total_transfer = datetime.datetime.now() - start_transfer
                io.summary = stMsg + ': Diff successful.' + " Transfer time: " + str(total_transfer)
            else:
                # Files do not match
                io.status = nap.CRITICAL
